                                    ...
                                    ScriptEngine *eng, bool *matched) const
{
    ...
    Instance ret(_altRefType.toInstance(inst->vmem(), inst, fac, name,
                                        parentNames));

    // Check for pointers that point back into or overlap with "inst"
    bool overlap;
    if (ret.address() < inst->address())
        overlap = (inst->address() <= ret.endAddress());
    else
        overlap = (ret.address() <= inst->endAddress());

    if (overlap && !members.isEmpty()) {
        ...
        ret = *inst;
        Instance tmp;
        // Find out where the pointer points to natively (without expressions)
        for (int i = 0; i < members.size(); ++i) {
