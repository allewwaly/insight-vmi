...
displayRecognitionError	    (pANTLR3_BASE_RECOGNIZER recognizer, pANTLR3_UINT8 * tokenNames)
{
    ...
    fprintf(stderr, "%lld) ", (long long int)recognizer->exception->line);
#else
	...
	case	ANTLR3_MISMATCHED_SET_EXCEPTION:
		...
		if  (size > 0)
		{
			...
			if  (tokenNames[bit])
			{
			    fprintf(stderr, "%s%s", count > 0 ? ", " : "", tokenNames[bit]);
			    count++;
			}
		    }
		    fprintf(stderr, "\n");
...
recoverFromMismatchedElement	    (pANTLR3_BASE_RECOGNIZER recognizer, pANTLR3_BITSET follow)
{
	 ...
	 */
	newFollow   = follow->orOp(follow, viableToksFollowingRule);
